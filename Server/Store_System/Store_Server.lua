...
function StoreHandler.UpdateCurrencies(player)
	...
	for currencyId, currency in pairs(GetCurrencyData()) do
		...
		if currencyTypeText == "VOTE_POINT" then
			val = player:GetVotePoints()
		end

		if currencyTypeText == "DONATION_POINT" then
			val = player:GetDonationPoints()
		end
		...
		if serviceHandler then
			...
			if success then
				...
				player:SendAreaTriggerMessage(
					services[serviceId][KEYS.service.name] .. " " .. CONFIG.strings.successfulPurchase
				)
			end
		end
	end
...
function SHOP_UI.DeductCurrency(player, currencyId, amount)
	...
	if CURRENCY_TYPES[currencyType] == "GOLD" then
		if player:GetCoinage() < amount * 10000 then
			player:SendAreaTriggerMessage(
				"|cFFFF0000" .. CONFIG.strings.insufficientFunds .. " " .. currencyName .. "|r"
			)
	...
	if CURRENCY_TYPES[currencyType] == "VOTE_POINT" then
		if player:GetVotePoints() < amount then
			player:SendAreaTriggerMessage(
				"|cFFFF0000" .. CONFIG.strings.insufficientFunds .. " " .. currencyName .. "|r"
			)
			player:PlayDirectSound(GetSoundEffect("notEnoughMoney", player:GetRace(), player:GetGender()), player)
		...
		player:ModifyVotePoints(-amount)
	end

	if CURRENCY_TYPES[currencyType] == "DONATION_POINT" then
		if player:GetDonationPoints() < amount then
			player:SendAreaTriggerMessage(
				"|cFFFF0000" .. CONFIG.strings.insufficientFunds .. " " .. currencyName .. "|r"
			)
			player:PlayDirectSound(GetSoundEffect("notEnoughMoney", player:GetRace(), player:GetGender()), player)
		...
		player:ModifyDonationPoints(-amount)
	end
...
function SHOP_UI.LogPurchase(player, data)
	...
	WorldDBExecute(
		"INSERT INTO store_logs(account, guid, serviceId, currencyId, cost) VALUES("
			.. player:GetAccountId()
			.. ", "
			.. player:GetGUIDLow()
			.. ", "
...
function SHOP_UI.ItemHandler(player, data)
	...
	-- Send reward mail
	SendMail(
		"Purchase of: " .. data[KEYS.service.name],
		CONFIG.strings.mailBody,
		player:GetGUIDLow(),
		CONFIG.mailSenderGUID,
		62,
		0,
		0,
		0,
...
function SHOP_UI.PetHandler(player, data)
	...
	-- check if player already has the spells learned
	if knownCount == rewardCount then
		player:SendAreaTriggerMessage("|cFFFF0000" .. CONFIG.strings.alreadyKnown .. " known|r")
		player:PlayDirectSound(GetSoundEffect("cantLearn", player:GetRace(), player:GetGender()), player)
		return false
	end
