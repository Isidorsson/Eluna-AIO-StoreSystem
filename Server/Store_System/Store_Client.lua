...
function SHOP_UI.MainFrame_Create()
	...
	shopFrame.CloseButton:SetScript("OnClick", function()
		MainFrame_Toggle()
	end)
	...
	shopFrame:SetScript("OnHide", function()
...
function SHOP_UI.ServiceBoxes_Create(parent)
	...
	for i = 1, 8 do
		...
		service:SetScript("OnClick", function(self)
...
function SHOP_UI.PageButtons_Create(parent)
	...
	forwardButton
		:GetNormalTexture()
		:SetTexCoord(CoordsToTexCoords(1024, forwTopX + 31, forwTopY, forwBotX + 31, forwBotY))
	forwardButton
		:GetPushedTexture()
		:SetTexCoord(CoordsToTexCoords(1024, forwTopX + 62, forwTopY, forwBotX + 62, forwBotY))
...
function SHOP_UI.CurrencyBadges_Create(parent)
	...
	for i = 1, 4 do
		...
		currencyButton:SetScript("OnLeave", function(self)
			GameTooltip:Hide()
		end)
...
function SHOP_UI.ModelFrame_Create(parent)
	...
	modelFrame:SetScript("OnHide", function()
	...
	modelFrame.playerModel:SetScript("OnMouseDown", function(self, button)
		local startPos = {
			GetCursorPosition(),
		}
	...
	modelFrame.playerModel:SetScript("OnMouseUp", function(self, button)
		self:SetScript("OnUpdate", nil)
	end)

	modelFrame.playerModel:SetScript("OnMouseWheel", function(self, zoom)
		local pos = {
			self:GetPosition(),
		}
		if zoom == 1 then
	...
	modelFrame.creatureModel:SetScript("OnMouseDown", function(self, button)
		local startPos = {
			GetCursorPosition(),
		}
		if button == "LeftButton" then
	...
	modelFrame.creatureModel:SetScript("OnMouseUp", function(self, button)
		self:SetScript("OnUpdate", nil)
	end)

	modelFrame.creatureModel:SetScript("OnMouseWheel", function(self, zoom)
		local pos = {
			self:GetPosition(),
		}
		if zoom == 1 then
	...
	modelFrame.leftButton:SetScript("OnMouseDown", function(self, button)
	...
	modelFrame.leftButton:SetScript("OnMouseUp", function(self, button)
		self:SetScript("OnUpdate", nil)
	end)
	...
	modelFrame.rightButton:SetScript("OnMouseDown", function(self, button)
		PlaySound("igInventoryRotateCharacter")
		self:SetScript("OnUpdate", function(self, elapsed)
			if modelFrame.playerModel:IsShown() then
				modelFrame.playerModel:SetFacing(modelFrame.playerModel:GetFacing() - 0.03)
	...
	modelFrame.rightButton:SetScript("OnMouseUp", function(self, button)
		self:SetScript("OnUpdate", nil)
	end)
