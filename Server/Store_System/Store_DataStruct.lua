...
function NavData.Load()
	...
	local Query = WorldDBQuery("SELECT * FROM store_categories")
	if Query then
		repeat
...
function CurrencyData.Load()
	...
	local Query = WorldDBQuery("SELECT * FROM store_currencies")
	if Query then
		repeat
...
function ServiceData.Load()
	...
	local Query = WorldDBQuery("SELECT * FROM store_services;")
	if Query then
		repeat
...
function CreatureDisplays.Load()
	...
	for k, v in pairs(ServiceData.Cache) do
		if
			(v[KEYS.service.serviceType] == 3 or v[KEYS.service.serviceType] == 4)
			and v[KEYS.service.displayOrEntry] > 0
		then
	...
	if GetCoreName() == "TrinityCore" then
		Query = WorldDBQuery(
			"SELECT entry, `name`, subname, IconName, type_flags, `type`, family, `rank`, KillCredit1, KillCredit2, HealthModifier, ManaModifier, RacialLeader, MovementType, modelId1, modelId2, modelId3, modelId4 FROM creature_template WHERE entry IN ("
				.. tmp
				.. ");"
		)
	elseif GetCoreName() == "AzerothCore" then
		Query = WorldDBQuery(
			"SELECT entry, `name`, subname, IconName, type_flags, `type`, family, `rank`, KillCredit1, KillCredit2, HealthModifier, ManaModifier, RacialLeader, MovementType FROM creature_template WHERE entry IN ("
				.. tmp
				.. ");"
		)
	end
	...
	if GetCoreName() == "AzerothCore" then
		local ModelQuery = WorldDBQuery(
			"SELECT CreatureID, Idx, CreatureDisplayID FROM creature_template_model WHERE CreatureID IN ("
				.. tmp
				.. ");"
		)
		if ModelQuery then
...
function LinkData.Load()
	...
	local Query = WorldDBQuery("SELECT * FROM store_category_service_link;")
	if Query then
		repeat
